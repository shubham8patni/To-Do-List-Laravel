<div>
    ...
    <div class="form-group row">
        ...
        <div style="display: flex;justify-content: center; width: 100%;" wire:key="{{$loop->index}}">
            @if(isset($step['name']))
            <input type="text" name="stepName[]" style="width: 100%;  margin-top: 5px; margin-bottom: 5px;" required="true" placeholder="{{'step '.($loop->index + 1)}}" value="{{$step['name']}}">

            <input type="hidden" name="stepId[]" value="{{$step['id']}}">
            @else
            <input type="text" name="stepName[]" style="width: 100%;  margin-top: 5px; margin-bottom: 5px;" required="true" placeholder="{{'step '.($loop->index + 1)}}" >
